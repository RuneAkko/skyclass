...
public class MediaController extends FrameLayout {
	...
	public String getStartPauseTime() {
		return startPauseTime;
	}

	public void setStartPauseTime(String startPauseTime) {
	...
	private long startTrackTimeForVideo;
	...
	public long getAllpauselen() {
		return allpauselen;
	}

	public void setAllpauselen(long allpauselen) {
		this.allpauselen = allpauselen;
	}
	...
	public void setAnchorView(View view) {
		...
		if (!mFromXml) {
			...
			mWindow.setHeight(android.view.ViewGroup.LayoutParams.MATCH_PARENT); //Android 7 MediaController显示
		}
		initControllerView(mRoot);
	}
	...
	//读入数据库操作数据
	public void setParameters(Boolean isLive,String studentcode, String courseid, String coursewareid){
		this.isLive = isLive;
	...
	private void doPauseResume() {
		if (mPlayer.isPlaying())
		{
			mPlayer.pause();
			isPaused = true;
			startPauseTime = StringUtility.getDateTime();
			//数据库操作
			System.out.println("暂停学习："+StringUtility.getDateTime()+"      "+mCurrentTime.getText());
			//usrService.save(new User(usrName,courseId,coursewareId,"暂停学习",StringUtility.getDateTime()));
		}
		else
		{
			mPlayer.start();
			...
			//数据库操作
			if(!isLive){
			System.out.println("继续学习："+StringUtility.getDateTime()+"     "+mCurrentTime.getText());
			long pauselen = 0;
			try {
				if(startPauseTime !=null)
				{
				date1=df.parse(startPauseTime);
				date2=df.parse(StringUtility.getDateTime());
				pauselen=(date2.getTime() - date1.getTime())/1000;
				allpauselen = allpauselen+pauselen;}
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				System.out.println("解析错误");
				e.printStackTrace();
			}
			usrService.save(new User("3",studentcode,courseId,coursewareId,startPauseTime,pauselen));
			System.out.print("暂停时长"+pauselen+"    "+allpauselen);
			}
		}
		updatePausePlay();
	}
    ...
    public void onStartTrackingTouch(SeekBar bar) {
			...
			if(!isLive)
			{
			 startTrackTime=StringUtility.getDateTime();
			 if(isPaused) {
					isPaused = false;
				long pauselen = 0;
				try {
					date1=df.parse(startPauseTime);
					date2=df.parse(startTrackTime);
					pauselen=(date2.getTime() - date1.getTime())/1000;
					allpauselen = allpauselen+pauselen;
					System.out.println("暂停后直接拖动"+pauselen+"   "+allpauselen);
				} catch (ParseException e) {
					// TODO Auto-generated catch block
					System.out.println("解析错误");
					e.printStackTrace();
			 ...
			 startTrackTimeForVideo = mPlayer.getCurrentPosition();
			 System.out.println("拖动开始"+startTrackTime+"         "+startTrackTimeForVideo);
			}
		  }
    ...
    public void onStopTrackingTouch(SeekBar bar) {
			...
			//数据库操作
			if(!isLive){
			 long endTrackTimeForVideo = mPlayer.getCurrentPosition();
			 long len = (endTrackTimeForVideo - startTrackTimeForVideo)/1000;
			 System.out.println("拖动结束"+StringUtility.getDateTime()+"         "+endTrackTimeForVideo);
			 usrService.save(new User("4",studentcode,courseId,coursewareId,startTrackTime,len));
			}
		}
	};
